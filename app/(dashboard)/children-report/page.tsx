...
import { School, Megaphone, PartyPopper, Tent, Gift, Home, Star, Loader2 } from "lucide-react";
...
export default function ChildrenReportPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const data = await fetchDeptStats("Children Report");
      setStatsData(data);
      setLoading(false);
    };
    load();
  ...
  const stats = [
    { label: "Total Primary Schools", value: 112, color: "border-[#1b5e20]" },
    { label: "Active Bible Clubs", value: 45, color: "border-[#ffca28]" },
    { label: "Teachers / Volunteers", value: 86, color: "border-blue-500" },
  ];

  const subsidiaries = [
    {
      icon: <Megaphone className="text-[#1b5e20]" size={24} />,
      title: "Zonal Children's Rally",
      description: "Log attendance and decisions from zonal primary school rallies.",
      action: "Open Form",
      type: "children-rally"
    },
    {
      icon: <PartyPopper className="text-white" size={24} />,
    ...
    },
    {
      ...
      title: "Long Vacation Camp",
      description: "Statistics and spiritual outcomes of the annual children's holiday camp.",
      action: "Open Form",
      type: "children-camp"
    },
    {
      icon: <Gift className="text-[#1b5e20]" size={24} />,
      title: "Christmas Party Outreach",
      description: "Report on end-of-year evangelical children's parties.",
      action: "Open Form",
    ...
    },
    {
      ...
      description: "Data from community-based weekend Bible clubs.",
      action: "Open Form",
      type: "neighbourhood-bible-club"
    },
    {
      icon: <Star className="text-[#1b5e20]" size={24} />,
      title: "Week of Emphasis",
      description: "Record metrics for specialized children's week programs.",
      action: "Open Form",
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <div className="flex flex-col gap-1 mb-4 md:mb-8">
        ...
        <p className="text-gray-500 text-xs md:text-sm font-medium">Manage and record reports for all primary schools, neighbourhood clubs, and children's activities.</p>
      </div>
