...
import { Send, MapPin, Calendar, Trash2, Plus, Info, Users } from "lucide-react";
...
function TrainingsMeetingsContent() {
  ...
  const { register, control, handleSubmit, setValue } = useForm<TrainingFormValues>({
    defaultValues: {
      trainings: [{ description: "", venue: "", brothers: 0, sisters: 0, total: 0, cost: 0 }]
    }
  });
  ...
  // Watch brothers and sisters for each row to update total
  ...
  useEffect(() => {
    watchAllFields?.forEach((training, index) => {
      const b = Number(training.brothers) || 0;
      const s = Number(training.sisters) || 0;
      const total = b + s;
      if (total !== training.total) {
        setValue(`trainings.${index}.total` as any, total);
      }
    });
  ...
  const onSubmit = async (data: TrainingFormValues) => {
    ...
    try {
      const reportId = `tr_${auth.currentUser.uid}_${Date.now()}`;
      await setDoc(doc(db, "reports", reportId), {
        uid: auth.currentUser.uid,
        userName: auth.currentUser.displayName || "User",
        reportType: "Trainings & Meetings",
        zone,
        year,
        period,
        data,
      ...
      alert("Training records committed successfully!");
  ...
  const tdClass = "py-2 px-1 border-gray-100";
  ...
  return (
    <div className="flex-1 p-4 md:p-6 space-y-6 max-w-6xl mx-auto pb-24 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 border-b pb-6 border-gray-100">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-[#1b5e20] tracking-tight">Trainings, Seminars & Committee Meetings</h1>
          <p className="text-gray-500 font-medium text-sm md:text-base">Track leadership development programs, staff training execution metrics, and attendance indicators.</p>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card className="border-none shadow-sm overflow-hidden">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-gray-800 uppercase tracking-tight text-sm">Program Matrix Execution</h3>
            </div>
            <Button 
                type="button" 
                variant="outline" 
                size="sm"
                onClick={() => append({ description: "", venue: "", brothers: 0, sisters: 0, total: 0, cost: 0 })}
                className="text-xs font-bold border-[#1b5e20] text-[#1b5e20] hover:bg-[#1b5e20] hover:text-white"
            >
              <Plus className="w-3 h-3 mr-1" /> Add Program Row
            </Button>
          </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className={thClass}>Venue</TableHead>
                    <TableHead className={thClass}>Brothers</TableHead>
                    <TableHead className={thClass}>Sisters</TableHead>
                    <TableHead className={thClass}>Total</TableHead>
                    <TableHead className={thClass}>Cost (₦)</TableHead>
                  ...
                  {fields.map((field, index) => (
                    <TableRow key={field.id} className="hover:bg-gray-50/30 transition-colors">
                      <TableCell className={tdClass}>
                        <Input 
                            placeholder="e.g. Visitors Seminar" 
                      ...
                      <TableCell className={tdClass}>
                         <Input 
                            ...
                            {...register(`trainings.${index}.venue`)} 
                            className="text-xs h-9 border-none bg-transparent focus:ring-0 px-2"
                        />
                      </TableCell>
                      <TableCell className={tdClass}>
                         <Input 
                            type="number" 
                            {...register(`trainings.${index}.brothers`)} 
                            className="text-xs h-9 border-none bg-transparent focus:ring-0 px-2 text-center"
                        />
                      </TableCell>
                      <TableCell className={tdClass}>
                         <Input 
                            type="number" 
                            {...register(`trainings.${index}.sisters`)} 
                            className="text-xs h-9 border-none bg-transparent focus:ring-0 px-2 text-center"
                        />
                      </TableCell>
                      <TableCell className={tdClass}>
                         <Input 
                            type="number" 
                            readOnly 
                            {...register(`trainings.${index}.total`)} 
                            className="text-xs h-9 border-none bg-gray-50 focus:ring-0 px-2 text-center font-bold"
                        />
                      </TableCell>
                      <TableCell className={tdClass}>
                         <Input 
        ...
        <div className="fixed bottom-0 left-0 lg:left-64 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-gray-200 shadow-lg flex justify-center z-50">
          <Button type="submit" disabled={isSubmitting} className="w-full max-w-md h-12 text-lg font-black bg-[#1b5e20] hover:bg-[#2e7d32] shadow-xl uppercase tracking-widest">
            {isSubmitting ? "Submitting..." : <><Send className="w-5 h-5 mr-3" /> Commit Training Records</>}
          </Button>
        </div>
      </form>
    </div>
...
export default function TrainingsMeetingsPage() {
  return (
    <Suspense fallback={<div className="flex-1 p-6">Loading trainings report...</div>}>
